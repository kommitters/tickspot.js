import axios from 'axios';
import { togglError, togglResponse } from '../../fixture/clients/toggl/listResponse.js';

import getEntries from '../../../src/clients/toggl/entries/list.js';

jest.mock('axios');

describe('togglAuth', () => {
  describe('when API call is successful', () => {
    it('should return the toggl data', async () => {
      axios.get.mockResolvedValueOnce(togglResponse);
      return expect(getEntries()).resolves.toBe(togglResponse.data);
    });
  });
  describe('when API call is not succesful', () => {
    it('Should reject with an error when authentication fails', async () => {
      axios.get.mockRejectedValueOnce(togglError);
      return expect(getEntries()).resolves.toBe(togglError);
    });
  });
});
